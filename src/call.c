...
static bool detect_hold_resume(enum bevent_ev *ev, struct call *call,
			       const enum sdp_dir old_rdir, bool is_offer)
{
	...
	bool old_on_hold = (old_rdir == SDP_RECVONLY ||
			    old_rdir == SDP_INACTIVE);
	bool new_on_hold = (new_rdir == SDP_RECVONLY ||
	...
	if (is_offer) {
		if (!old_on_hold && new_on_hold) {
			*ev = BEVENT_CALL_HOLD;
			call->remote_hold = true;
			emit_event = true;
		}
		else if (old_on_hold && !new_on_hold) {
			*ev = BEVENT_CALL_RESUME;
			call->remote_hold = false;
			emit_event = true;
		}
	}
	else if (call->remote_hold && !new_on_hold) {
...
static void sipsess_estab_handler(const struct sip_msg *msg, void *arg)
{
	...
	if (call->audio) {
		const struct sdp_media *m =
			stream_sdpmedia(audio_strm(call->audio));
		const enum sdp_dir rdir = sdp_media_rdir(m);
		call->remote_hold = (rdir == SDP_RECVONLY ||
				     rdir == SDP_INACTIVE);
	}
